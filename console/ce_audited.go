...
func handleCEInvokeAudited(w http.ResponseWriter, r *http.Request) {
	...
	//        (server prefetch + single Vultr call). Default v2 per WP-AO-63.
	vultrKey := os.Getenv("VULTR_INFERENCE_API_KEY")
	if vultrKey == "" {
		envelope.Status = "ce_error"
		envelope.Error = "VULTR_INFERENCE_API_KEY not configured on server"
		envelope.TotalDurationMs = time.Since(totalStart).Milliseconds()
		writeAuditedJSON(w, envelope, http.StatusInternalServerError)
		return
	}
