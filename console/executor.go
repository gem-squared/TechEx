...
func resolveCrafterModel(sess *SessionData) string {
	...
	// WP-AO-48: default Crafter LLM is gemini-2.5-pro — better tool-calling
	// reliability + less hallucination on skill-execution prompts than Flash.
...
// agentGenerate routes to Gemini or Vultr depending on the model prefix.
func agentGenerate(agentModel, prompt string) (string, error) {
	if isVultrModel(agentModel) {
		...
		// WP-AO-42: skill execution needs markdown output. Use the freeform
		// variant which does NOT set response_format=json_object. JSON-mode
		// stays available for CE invocation + sample-input generation via
		// vultrSheepCall.
