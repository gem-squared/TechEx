...
// runCEv2 is the shared v2 execution: prefetch → vultrSheepCall → parse JSON.
...
func runCEv2(ctx context.Context, spec *CESpec, input any) (parsed any, raw string, trace []ToolCallTrace, durations map[string]int64, err error) {
	...
	execStart := time.Now()
	rawText, callErr := vultrSheepCall(vultrKey, vultrAPIModelID(model), systemPrompt, userPrompt)
